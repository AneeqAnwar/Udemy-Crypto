...
class Attack:
    ...
    def calculate_freq(self, cipher):
        ...
        letter_count = 0
        for c in cipher:
            if c in self.frequency:
                self.frequency[c] += 1
                letter_count += 1

        for c in self.frequency:
            self.frequency[c] = round(self.frequency[c]/letter_count, 4)

    def print_freq(self):
        new_line_count = 0

        for c in self.frequency:
            print(c, ':', self.frequency[c], ' ', end='')
    ...
    def calculate_matches(self):
        for cipher_char in self.alphabet:
            ...
            for plain_char in self.alphabet:
                map[plain_char] = round(abs(self.frequency[cipher_char] - self.freq_eng[plain_char]), 4)
